    ...
    <div className="min-h-screen bg-luxury-bg flex flex-col justify-between overflow-x-hidden relative">
      ...
      <header className="relative z-20 px-6 py-6 border-b border-theme-subtle bg-overlay-header backdrop-blur-md flex items-center justify-between">
        ...
        <div className="text-center">
          <h2 className="text-base font-semibold text-primary">العروض الخاصة</h2>
          <p className="text-[10px] text-gold-primary tracking-widest font-semibold">SPECIAL OFFERS</p>
        </div>
      ...
      <main className="flex-1 w-full max-w-5xl mx-auto px-6 py-10 flex flex-col justify-center space-y-6 md:space-y-8">
        {isLoading ? (
          <div className="space-y-6">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-48 md:h-60 rounded-2xl border border-theme-subtle bg-overlay-skeleton animate-pulse" />
            ))}
          </div>
        ) : offers.length === 0 ? (
          <div className="text-center py-20">
            <p className="text-gray-500 text-sm font-medium">لا توجد عروض خاصة متاحة حالياً</p>
          </div>
        ) : (
          offers.map((offer, idx) => (
            <motion.div
              ...
              className="relative h-48 md:h-60 rounded-2xl overflow-hidden border border-theme-subtle hover:border-gold-primary/20 transition-all duration-500 shadow-xl bg-gradient-to-br from-gold-primary/10 via-overlay-card to-overlay-card"
            >
